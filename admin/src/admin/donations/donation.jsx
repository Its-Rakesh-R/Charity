...
function Donation(){
    ...
    let i =1
    useEffect(()=>{
        axios.get("http://localhost:3003/admin/donation")
        .then((res)=>{
            setdata(res.data)
        })
        .catch((err) => {
        ...
        </div>
    {
                data.map((e)=>{
                    return <div key={e._id} className=' table_row'  to={`/sponser2/${e._id}`} >
                        ...
                        <div className=' table_cell'>{e.address?(e.address):('Payment mode is '+e.payment_method)}</div>
                        <div className=' table_cell'>{e.collection_date}</div>
                        <div className=' table_cell'>{e.category}</div>
                        <div className=' table_cell'>{e.gadget?(e.gadget):(e.material || '₹ '+e.amount)}</div>
